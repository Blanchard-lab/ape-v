...
'''
Process: 
Image -> CNN embeddings -> pooling: (can have lstm, current code, input dim 2048) -> fc layer -> class

(N, 2048) -> (1, 2048) -> (1, 2) -> argmax(softmax) == label
'''
...
space = {
    'batch_size': hp.choice('batch_size', [8, 16, 32]), # 128, 256, 512
    'num_epochs': hp.choice('num_epochs', range(5, 50, 5)),
    'unfreeze_layers': hp.choice('unfreeze_layers', range(0, 5, 1)), # max 6 possible for resnet18
...
def objective(h):
    ...
    args.unfreeze_layers = h['unfreeze_layers']
    ...
    # Load whole dataset
    data = pd.read_csv(r"/s/babbage/b/nobackup/nblancha/public-datasets/APE-V/Experiment_2/Exp_2.1/video_paths_labels_full_rsz_fps15_center_10.csv")
    ...
    for fold in range(k):
        ...
        #gpu_usage(args.gpu)
        # Load data
        data_train, data_valid = get_k_fold_data(k, fold, data, pid_videos) # Get k-fold cross-validation training and verification data
        data_train_labels = torch.tensor(data_train['label'].values)
        data_valid_labels = torch.tensor(data_valid['label'].values)
        ...
        test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, num_workers=args.workers, shuffle=False)

        #gpu_usage(args.gpu)
        '''
        print(f'train_lengths: {train_lengths}')
        print(f'max train_lengths: {max(train_lengths)}')
        print(f'min train_lengths: {min(train_lengths)}')
        print(f'avg train_lengths: {np.mean(train_lengths)}')
        print(f'test_lengths: {test_lengths}')
        print(f'max test_lengths: {max(test_lengths)}')
        print(f'min test_lengths: {min(test_lengths)}')
        print(f'avg test_lengths: {np.mean(test_lengths)}')
        '''
        
        model = CNNLSTMModel(args, args.size, h['n_hidden_units'], h['n_hidden_layers'], num_classes=args.num_classes)
        #model = CNNLSTMModel(args, args.size, 200, 4, num_classes=args.num_classes)
        if args.gpu is not None:
            torch.cuda.set_device(args.gpu)
            model = model.cuda(args.gpu)

        num_params = []
        num_params.append(sum(p.numel() for p in list(model.parameters()) if p.requires_grad))
        '''
        print(("""
                   model Configurations:
                       ...
                       number of parameters: {}
                                   """.format(args.size, args.num_classes, h['n_hidden_units'], h['n_hidden_layers'], h['batch_size'], num_params)))
        '''
        ...
        model_name = f'athletic_model_video_{model_count_fold}.pt'
        ...
        for epoch in range(num_epochs):
            ...
            y_true, y_pred, loss_val, acc_val = validate(test_loader, model, epoch)
        ...
        f.write('\n')
        for i, tl, pl in zip(range(len(data_valid)), save_true_labels, save_predicted_labels):
            f.write(str(f'{data_valid.loc[i, "participant"]}_{data_valid.loc[i, "type"]}_{data_valid.loc[i, "jump_count"]}'))
            f.write(',')
            f.write(str(tl))
            f.write(',')
            f.write(str(pl))
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (frames, labels, lengths) in enumerate(train_loader):
        batch_num = i + 1
        # Clear GPU cache
        ...
        # print('size of frames is: ', frames.size())
        ...
        top1.update(acc1[0], frames.size(0))
...
def validate(test_loader, model, epoch):
    ...
    with torch.no_grad():
        for j, (frames, labels, lengths) in enumerate(test_loader):
            #batch_num = j+1
            # Load frames and labels onto GPU
            if args.gpu is not None:
                frames = frames.cuda(args.gpu, non_blocking=True)
