...
import random
import statistics
from model import CNNLSTMModel, CNNLSTMModelEnsemble
...
def create_hyperparameters_results_file():    
    ...
    file_path = fr'hyperparameters_athletics_video_guided_combined.csv'
    f = open(file_path, 'a+')
    f.write(headings)
    f.write('\n')
    f.close()
...
space = {
    'batch_size': hp.choice('batch_size', [8, 16, 32]), # 16, 32, 64, 128, 256, 512
    'num_epochs': hp.choice('num_epochs', range(5, 50, 5)),
    'learning_rate': hp.loguniform('learning_rate', np.log(1e-5), np.log(1e-2)) # LR bw 0.00001 and 0.01
}
...
def objective(h):
    ...
    for fold in range(k):
        ...
        hidden_dim_center = 10
        hidden_dim_left = 10
        hidden_dim_right = 30
        model_center = CNNLSTMModel(args.size, hidden_dim=hidden_dim_center, layer_dim=4, num_classes=args.num_classes)
        model_left = CNNLSTMModel(args.size, hidden_dim=hidden_dim_left, layer_dim=3, num_classes=args.num_classes)
        model_right = CNNLSTMModel(args.size, hidden_dim=hidden_dim_right, layer_dim=3, num_classes=args.num_classes)
        # Load state dicts
        model_center.load_state_dict(torch.load(fr'best_model_center/athletic_model_video_66_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(args.gpu)))
        model_left.load_state_dict(torch.load(fr'best_model_left/athletic_model_video_61_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(args.gpu)))
        model_right.load_state_dict(torch.load(fr'best_model_right/athletic_model_video_20_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(args.gpu)))
        
        # Create ensemble model
        model = CNNLSTMModelEnsemble(model_center, hidden_dim_center, model_left, hidden_dim_left, model_right, hidden_dim_right, num_classes=args.num_classes)
...
def validate(test_loader_center, test_loader_left, test_loader_right, model, epoch):
    ...
    with torch.no_grad():
        for (i, (frames_center, labels, lengths_center)), (j, (frames_left, _, lengths_left)), (k, (frames_right, _, lengths_right)) in zip(enumerate(test_loader_center), enumerate(test_loader_left), enumerate(test_loader_right)):
            #batch_num = j+1
            # Load frames and labels onto GPU
            frames_center = load_data_gpu_transpose(frames_center)
            frames_left = load_data_gpu_transpose(frames_left)
...
def run_search():
    output = fmin(fn=objective,
                space=space,
                algo=partial(tpe.suggest, n_startup_jobs=10), # Guided Search
