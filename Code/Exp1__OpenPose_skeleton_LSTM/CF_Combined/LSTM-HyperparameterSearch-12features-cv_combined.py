...
class LSTMModelEnsemble(nn.Module):
    def __init__(self, model_center, hidden_dim_center, layer_dim_center, model_left, hidden_dim_left, layer_dim_left, model_right, hidden_dim_right, layer_dim_right, num_classes):
        super(LSTMModelEnsemble, self).__init__()
        self.num_classes = num_classes
        
        # Hidden dims and layers
        self.hidden_dim_center = hidden_dim_center
        self.layer_dim_center = layer_dim_center
        self.hidden_dim_left = hidden_dim_left
        self.layer_dim_left = layer_dim_left
        self.hidden_dim_right = hidden_dim_right
        self.layer_dim_right = layer_dim_right
        ...
        # FC layer (Classifier)
        # check out.size()
        self.fc1 = nn.Linear(self.hidden_dim_center+self.hidden_dim_left+self.hidden_dim_right, self.num_classes)
        
    def model_forward(self, model, hidden_dim, layer_dim, x, seq_len):
        # Tensors declared in forward() have to be manually transferred on GPU
    
        # Initialize hidden state with zeros
        h0 = torch.zeros(layer_dim, x.size(0), hidden_dim).requires_grad_().to(device)

        # Initialize cell state
        c0 = torch.zeros(layer_dim, x.size(0), hidden_dim).requires_grad_().to(device)
        
        # Get lstm output
        #self.lstm.flatten_parameters() # For DataParallel
        for child in model.children():
            out, (hn, cn) = child(x, (h0, c0))
    ...
    def forward(self, frames_center, lengths_center, frames_left, lengths_left, frames_right, lengths_right):
        frames_center_op = self.model_forward(self.model_center, self.hidden_dim_center, self.layer_dim_center, frames_center, lengths_center) # Forward pass for model center
        frames_left_op = self.model_forward(self.model_left, self.hidden_dim_left, self.layer_dim_left, frames_left, lengths_left) # Forward pass for model left
        frames_right_op = self.model_forward(self.model_right, self.hidden_dim_right, self.layer_dim_right, frames_right, lengths_right) # Forward pass for model right
        
        # Concatenate outputs for fc layer
        input_to_fc = torch.Tensor([]).to(device)
        for t in range(frames_center_op.size(0)):
...
def create_hyperparameters_results_file():    
    ...
    file_path = fr'hyperparameters_athletics_12features_guided_combined.csv'
    f = open(file_path, 'a+')
    f.write(headings)
    f.write('\n')
    f.close()
...
space = {
    ...
    'num_epochs': hp.choice('num_epochs', range(10, 255, 5)),
    'learning_rate': hp.loguniform('learning_rate', np.log(1e-4), np.log(1e-1)) # LR bw 0.0001 and 0.1
}
...
def load_data(X_train, y_train, X_valid, y_valid, batch_size):
    ...
    return train_dataset, train_loader, test_dataset, test_loader, train_length_of_videos, test_length_of_videos
...
def objective(h):
    ...
    # Load whole dataset
    X_center = pd.read_csv(r"video_frames_12features.csv")
    y_center = pd.read_csv(r"labels.csv")
    X_left = pd.read_csv(r"video_frames_12features_left.csv")
    y_left = pd.read_csv(r"labels_left.csv")
    X_right = pd.read_csv(r"video_frames_12features_right.csv")
    y_right = pd.read_csv(r"labels_right.csv")
    ...
    for fold in range(k):
        ...
        X_train_center, y_train_center, X_valid_center, y_valid_center = get_k_fold_data(k, fold, X_center, y_center, pid_videos) # Center
        X_train_left, y_train_left, X_valid_left, y_valid_left = get_k_fold_data(k, fold, X_left, y_left, pid_videos) # Left
        X_train_right, y_train_right, X_valid_right, y_valid_right = get_k_fold_data(k, fold, X_right, y_right, pid_videos) # Right
        
        # Create dataloaders
        train_dataset_center, train_loader_center, test_dataset_center, test_loader_center, train_length_of_videos_center, test_length_of_videos_center = load_data(X_train_center, y_train_center, X_valid_center, y_valid_center, batch_size) # Center angle
        train_dataset_left, train_loader_left, test_dataset_left, test_loader_left, train_length_of_videos_left, test_length_of_videos_left = load_data(X_train_left, y_train_left, X_valid_left, y_valid_left, batch_size) # Left angle
        train_dataset_right, train_loader_right, test_dataset_right, test_loader_right, train_length_of_videos_right, test_length_of_videos_right = load_data(X_train_right, y_train_right, X_valid_right, y_valid_right, batch_size) # Right angle
        
        # Create models (Use hidden units and layers of best model of each angle)
        hidden_dim_center = 50
        layer_dim_center = 1
        hidden_dim_left = 80
        layer_dim_left = 4
        hidden_dim_right = 165
        layer_dim_right = 2
        
        model_center = LSTMModel(input_dim, hidden_dim_center, layer_dim_center, output_dim)
        model_left = LSTMModel(input_dim, hidden_dim_left, layer_dim_left, output_dim)
        model_right = LSTMModel(input_dim, hidden_dim_right, layer_dim_right, output_dim)
        
        # Load state dicts
        model_center.load_state_dict(torch.load(fr'best_model_center/athletic_model_12features_94_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(device)))
        model_left.load_state_dict(torch.load(fr'best_model_left/athletic_model_12features_134_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(device)))
        model_right.load_state_dict(torch.load(fr'best_model_right/athletic_model_12features_21_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(device)))
        
        # Use all available GPUs
        #model= nn.DataParallel(model)
        # Place model on GPU
        model_center = model_center.cuda(device)
        model_left = model_left.cuda(device)
        model_right = model_right.cuda(device)
        
        # Create ensemble model
        model = LSTMModelEnsemble(model_center, hidden_dim_center, layer_dim_center, model_left, hidden_dim_left, layer_dim_left, model_right, hidden_dim_right, layer_dim_right, num_classes=output_dim)
        ...
        train_seq_dim_center = max(train_length_of_videos_center) #max_length of frames from videos # time steps (number of frames in video)
        test_seq_dim_center = max(test_length_of_videos_center)
        train_seq_dim_left = max(train_length_of_videos_left) 
        test_seq_dim_left = max(test_length_of_videos_left)
        train_seq_dim_right = max(train_length_of_videos_right) 
        test_seq_dim_right = max(test_length_of_videos_right)
        #iter = 0
        ...
        save_true_labels = []
        model_name = f'athletic_model_12features_combined_{model_count}_{fold+1}.pt'
        ...
        for epoch in range(num_epochs):
            ...
            correct_train = 0
            total_train = 0
            for (i, (frames_center, labels, lengths_center)), (j, (frames_left, _, lengths_left)), (m, (frames_right, _, lengths_right)) in zip(enumerate(train_loader_center), enumerate(train_loader_left), enumerate(train_loader_right)):
                # Load frames as a torch tensor
                frames_center = frames_center.view(-1, train_seq_dim_center, input_dim)#.requires_grad_() # (num_samples, t, N)
                frames_left = frames_left.view(-1, train_seq_dim_left, input_dim)
                frames_right = frames_right.view(-1, train_seq_dim_right, input_dim)
                
                # Load frames and labels onto GPU
                frames_center, labels = frames_center.to(device), labels.to(device)
                frames_left = frames_left.to(device)
            ...
            correct_test = 0
            total_test = 0
            for (i, (frames_center, labels, lengths_center)), (j, (frames_left, _, lengths_left)), (m, (frames_right, _, lengths_right)) in zip(enumerate(test_loader_center), enumerate(test_loader_left), enumerate(test_loader_right)):
                frames_center = frames_center.view(-1, test_seq_dim_center, input_dim)#.requires_grad_() # (num_samples, t, N)
                frames_left = frames_left.view(-1, test_seq_dim_left, input_dim)
                frames_right = frames_right.view(-1, test_seq_dim_right, input_dim)
            ...
            train_l = running_loss_train / len(train_dataset_center)
            train_loss_values.append(train_l) # store all losses
            test_l = running_loss_test / len(test_dataset_center)
        ...
        f.write('\n')
        for i, tl, pl in zip(range(len(y_valid_center)), save_true_labels, save_predicted_labels):
            f.write(str(f'{y_valid_center.loc[i, "participant"]}_combined_{y_valid_center.loc[i, "jump_count"]}'))
            f.write(',')
            f.write(str(tl))
            f.write(',')
            f.write(str(pl))
...
def run_search():
    ...
    output = fmin(fn=objective,
                space=space,
                algo=partial(tpe.suggest, n_startup_jobs=25), # Guided Search
                max_evals=80
            )
    return output
