...
def objective(h):
    ...
    # Load whole dataset
    X_center = pd.read_csv(r"video_frames_kf_12features.csv")
    y_center = pd.read_csv(r"labels.csv")
    X_left = pd.read_csv(r"video_frames_kf_12features_left.csv")
    y_left = pd.read_csv(r"labels_left.csv")
    X_right = pd.read_csv(r"video_frames_kf_12features_right.csv")
    ...
    for fold in range(k):
        ...
        # Load state dicts
        model_center.load_state_dict(torch.load(fr'best_model_center/athletic_model_12features_84_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(device)))
        model_left.load_state_dict(torch.load(fr'best_model_left/athletic_model_12features_38_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(device)))
        model_right.load_state_dict(torch.load(fr'best_model_right/athletic_model_12features_34_{fold+1}.pt', map_location=lambda storage, loc: storage.cuda(device)))
