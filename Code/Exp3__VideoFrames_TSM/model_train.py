...
import random
import statistics
from model import CNNLSTMModel, VideoGlobalModel
...
space = {
    'batch_size': hp.choice('batch_size', [4, 8, 16]), # 64, 128, 256, 512
    'num_epochs': hp.choice('num_epochs', range(15, 80, 5)),
    'unfreeze_layers': hp.choice('unfreeze_layers', range(1, 7, 1)),
...
def objective(h):
    ...
    for fold in range(k):
        ...
        #print('Time r=taken to get the max length is: ', time.time() - st)
        ...
        #train_loader = torch.utils.data.DataLoader(dataset=train_dataset, sampler=BalancedBatchSampler(train_dataset, data_train_labels), batch_size=batch_size, num_workers=args.workers)
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, num_workers=args.workers, shuffle=True, drop_last=True)
        ...
        #test_loader = torch.utils.data.DataLoader(dataset=test_dataset, sampler=BalancedBatchSampler(test_dataset, data_valid_labels), batch_size=batch_size, num_workers=args.workers)
        test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, num_workers=args.workers, drop_last=True)
