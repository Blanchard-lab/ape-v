...
class VideoGlobalModelEnsemble(nn.Module):
    """
    This model contains only TSM module
    """

    def __init__(self, args, input_dim, model_center, model_left, model_right, num_classes):
        super(VideoGlobalModelEnsemble, self).__init__()
        ...
        # Add new classifier
        self.classifier = nn.Sequential(
            nn.Linear(512*3, self.coord_feature_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.coord_feature_dim, 512*3),
            nn.ReLU(inplace=True),
            nn.Linear(512*3, self.nr_actions)
        )
        
    def model_forward(self, model, global_img_input, seq_lengths):
        global_img_input = global_img_input.reshape(-1, 3*self.nr_frames, self.img_feature_dim, self.img_feature_dim)# (-1, num_channels*num_frames, self.input_dim, self.input_dim)
        org_features = torch.Tensor([]).cuda()
        videos_features = torch.Tensor([]).cuda()
        
        ct = 0
        for child in model.children():
            if ct == 1:
                org_features = child(global_img_input)  # [b, 2048, num_segments, 1, 1] # TSM
            elif ct == 4:
                videos_features = child(org_features)  # [b, 512, num_segments, 1, 1] # Conv
            ct += 1
        
        ct = 0        
        for child in model.children():
            if ct == 3:
                video_features = child(videos_features).squeeze()  # [b, 512] # avgpool
            ct += 1
        
        if len(seq_lengths) == 1:
            video_features = video_features.unsqueeze(0) # if batch_size = 1
        
        return torch.transpose(video_features, 0, 1)

    def forward(self, global_img_input_center, lengths_center, global_img_input_left, lengths_left, global_img_input_right, lengths_right):
        video_features_center_op = self.model_forward(self.model_center, global_img_input_center, lengths_center) # Forward pass for model center
        video_features_left_op = self.model_forward(self.model_left, global_img_input_left, lengths_left) # Forward pass for model left
        video_features_right_op = self.model_forward(self.model_right, global_img_input_right, lengths_right) # Forward pass for model right
        
        # Concatenate outputs for classifier
        video_features_combined = torch.Tensor([]).cuda()
        for t in range(video_features_center_op.size(1)):
            video_features_combined = torch.cat((video_features_combined, torch.cat((video_features_center_op[:, t], video_features_left_op[:, t], video_features_right_op[:, t]), dim=0).unsqueeze(1)), dim=1)
            
        video_features_combined = torch.transpose(video_features_combined, 0, 1)
        
        cls_output = self.classifier(video_features_combined)  # (b, num_classes)
        if len(lengths_center) == 1:
            return cls_output.unsqueeze(0) # if batch_size = 1
        else:
            return cls_output